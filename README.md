...
- 上部の月ボタンで各月へジャンプ。地層のように記録が貯まっていく
...
- **自動（スリープ復帰）**: 前回記録以降に新しい作業があった時だけ報告書を更新します。
  ブラウザでの表示は**1日1回まで**（その日すでに記録済みなら更新のみ行い、
  復帰のたびに画面が開くことはありません）。
...
**`git pull` だけで完了します。再インストールは不要です。**
...
ランチャーアプリ（`kiroku.app`）も sleepwatcher の `~/.wakeup` も、clone した
ディレクトリ内のスクリプトを直接参照する作りのため、pull した内容は次回の
実行から自動的に使われます。`install.sh` の再実行が必要になるのは、clone した
ディレクトリを移動・改名した場合だけです。

## 推奨: セッションログの保持期間を延ばす

kiroku の元データは Claude Code が残すセッションログ（`~/.claude/projects`）で、
Claude Code は既定でこれを**約 30 日で自動削除**します。1 か月以上 kiroku を
起動しなかった場合、削除済みの日の記録は復元できません。長期の起動忘れに
備えるには、`~/.claude/settings.json` に保持期間の設定を追加してください:
...
kiroku は前回記録より古いログファイルを開かずにスキップするため、保持期間を
延ばしても実行が遅くなることはありません。
...
- `docs/images/ogp.png` — ビューポート 1280×640、ズーム 1.15（サムネイルでも読めるように拡大）
- 撮影は fullPage ではなくビューポートのみ（下端がカードの途中で切れることで「まだ下に続く」ことを示すため）
- 出力先ディレクトリ（上記なら `/tmp`）で `python3 -m http.server` を起動し、127.0.0.1 経由で開く
